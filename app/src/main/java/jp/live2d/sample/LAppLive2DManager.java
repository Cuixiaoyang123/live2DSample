...
public class LAppLive2DManager {
	...
	public boolean tapEvent(float x, float y) {
		...
		for (int i = 0; i < models.size(); i++) {
			if (models.get(i).hitTest(LAppDefine.HIT_AREA_HEAD, x, y)) {
				Toast.makeText(context, "不要摸我的脸", Toast.LENGTH_SHORT).show();
				...
				Toast.makeText(context, "不要碰我的衣服", Toast.LENGTH_SHORT).show();
	...
	public void oneByOneExpression() {
		expressionNum += 1;
		String expression = "F0" + (expressionNum % 8);
		for (int i = 0; i < models.size(); i++) {
			models.get(i).setExpression(expression);//F01 ~ F07
	...
	public void showself() {
		if (LAppDefine.DEBUG_LOG) Log.d(TAG, "Showself event.");
		for (int i = 0; i < models.size(); i++) {
			models.get(i).startRandomMotion(LAppDefine.MOTION_GROUP_PINCH_OUT, LAppDefine.PRIORITY_FORCE);
		}
	}
